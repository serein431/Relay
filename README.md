...
ChatGPT 运行时不会立刻读取外部新增的本机任务。Relay 会在写入完成并检查任务记录后等待几秒，再发送打开请求。界面中的“已发送打开请求”只表示 macOS 已把链接交给 ChatGPT，不表示任务页面已经显示。如果 ChatGPT 仍提示找不到任务，请返回 Relay 点击“再次打开 ChatGPT”，不需要重新导入。
